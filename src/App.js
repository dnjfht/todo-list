...
// 그렇다면 왜 이렇게 동작을 하는 것인가? :
// useState가 호출될 때마다 함수를 호출해서 그 반환된 값을 전달하는 것 대신에
// 콜백함수를 전달하면 useState는 함수 하나만 만들어서 전달하고
// 초기값이 필요한 경우에만 전달한 함수를 호출해서 값을 얻을 거고,
// useState를 호출할 때 함수를 아무리 많이 만들어서 전달해도 내부적으로 가지고 있는 상태가 있다면
// 초기값이 필요하지 않으니까 초기값을 반환하는 함수를 호출하지 않을 것이다.
// 그래서 이 함수는 더이상 호출되지 않을 것이다!

// => 스테이트를 함수로 넣어 전달해주면(콜백함수) 기억하고 있는 값이 있을 경우 다시 호출되지 않는다!
